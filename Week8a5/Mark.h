...
namespace seneca {
   ...
   public:
      ...
      std::ostream& display(std::ostream& ost=std::cout);
      friend double operator/ (double number, Mark& mark);
      friend int operator/ (int number, Mark&mark);

   };
   
   // student helper function prototypes go here
   std::ostream& display(Mark& mark, char type, std::ostream& ost=std::cout);
   std::ostream& operator<< (std::ostream& ost, Mark& mark);
   std::istream& operator>>(std::istream& ist, Mark& mark);
   std::ifstream& operator>>(std::ifstream& ifs, Mark& mark);
   double operator+(double number, Mark& mark);
   int operator+(int number, Mark& mark);
   double operator-(double number, Mark& mark);
   int operator-(int number, Mark& mark);
